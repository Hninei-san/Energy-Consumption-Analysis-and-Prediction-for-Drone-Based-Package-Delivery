...
def calculate_wind_speed(dtime):
    """Calculates the mean air density for a given timestamp + 2 hours
    ...
    for met in metar_data:
        if time_in_range(dtime.time(), range_b, met.time.time()):
            ...
            #pressures.append(met.press.value(units="HPA"))
            try:
                wind_speed.append(met.wind_speed.value())
            except:
                wind_speed.append(-1)
            #wind_dir.append(met.wind_dir)

    #get the max value of the speed
    print(wind_speed)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        quit(1)
    dtime = datetime.datetime.strptime(f"{sys.argv[1]} {sys.argv[2]}", "%Y-%m-%d %H:%M")
    rho = calculate_density(dtime)
    print(rho)

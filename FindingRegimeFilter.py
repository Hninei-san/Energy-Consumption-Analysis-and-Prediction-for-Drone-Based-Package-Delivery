...
def Slopes(filteredData): # finds the slopes (first derivative)
	...
	takeOffStartIndex = 0
	while takeOffStartIndex < len(slopes) and slopes[takeOffStartIndex] <= 0.05:
		takeOffStartIndex += 1
	if takeOffStartIndex >= len(slopes):
		# All slopes are below threshold; fall back to the index of the max slope
		takeOffStartIndex = indexMax
...
def FindRegime(df):
	...
	'''
	df = df.reset_index()
	df["position_z"] = df["position_z"] - df["position_z"].min()
	filtered= Filter(df)
...
def main():
	...
	for flight in [201]:
		...
		ax1.grid(visible=True, which='major', color='gray', linewidth=1.0, alpha=0.1)
		ax1.set_xlim([40, 60])
		ax1.set_ylim([64, 78])

...
def main():
	...
	# Map columns from energy_summary_model1.csv
	# energy_summary.py uses Power_takeoff, Power_cruise, Power_landing and Pi_hover
	tk_x = df['Pi_hover']
	tk_y = df['Power_takeoff']
	tk_time = df['time_takeoff']

	cr_x = df['Pi_hover']
	cr_y = df['Power_cruise']
	cr_time = df['time_cruise']

	ld_x = df['Pi_hover']
	ld_y = df['Power_landing']
	ld_time = df['time_landing']
